...
PlanningNode::PlanningNode(const ros::NodeHandle& nh) : nh_(nh) {
  ...
  center_line_subscriber_ = nh_.subscribe("/center_line", 1, &PlanningNode::CenterLineCallback, this);
  obstacles_subscriber_ = nh_.subscribe("/obstacles", 1, &PlanningNode::ObstaclesCallback, this);
  dynamic_obstacles_subscriber_ = nh_.subscribe("/dynamic_obstacles", 1, &PlanningNode::DynamicObstaclesCallback, this);
  goal_subscriber_ = nh_.subscribe("/move_base_simple/goal", 1, &PlanningNode::PlanCallback, this);
...
void PlanningNode::CenterLineCallback(const CenterLineConstPtr& msg) {
  ...
  ROS_INFO("Planner: Centerline received");
}
...
void PlanningNode::ObstaclesCallback(const ObstaclesConstPtr& msg) {
  ...
  ROS_INFO("Planner: Obstacles received");
}
...
void PlanningNode::DynamicObstaclesCallback(const DynamicObstaclesConstPtr& msg) {
  ...
  ROS_INFO("Planner: Dynamic Obstacles received");
}
  ...
  if(planner_->Plan(state_, result)) {
    ...
    for (int i = 0; i < nfe; i++) {
      ...
      for (auto& obstacle: dynamic_obstacles) {
        ...
        visualization::PlotPolygon(obstacle.second, 0.2, visualization::Color::fromHSV(hue, 1.0, 1.0), obstacle.first, "Online Obstacle");
      }

      visualization::PlotConvexPolygon(convex_polygons[i], 0.1, visualization::Color::Cyan, 1, "Safe Corridors");
      visualization::PlotPoints(points_for_corridors[i], 0.3, visualization::Color::Cyan, 1, "Corridor Points");
...
void PlanningNode::PlotVehicle(const int id, const math::Pose& pt, const double phi) {
  ...
  for(auto &tire: tires) {
    visualization::PlotPolygon(math::Polygon2d(tire), 0.1, visualization::Color::White, id * (tire_id++), "Tires");
  }

  visualization::PlotPolygon(math::Polygon2d(config_.vehicle.GenerateBox({pt.x(), pt.y(), pt.theta()})), 0.2, visualization::Color::Yellow, id, "Footprint");
  visualization::Trigger();
}

std::array<math::Box2d, 4> PlanningNode::GenerateTireBoxes(const math::Pose& pose, const double phi) const {
  ...
  return {
    math::Box2d({pose.x() - rear_track_width_2 * sin_t, pose.y() + rear_track_width_2 * cos_t}, pose.theta(), box_length, 0.195),
    math::Box2d({pose.x() + rear_track_width_2 * sin_t, pose.y() - rear_track_width_2 * cos_t}, pose.theta(), box_length, 0.195),
    math::Box2d({front_pose.x() - front_track_width_2 * sin_t, front_pose.y() + front_track_width_2 * cos_t}, front_pose.theta() + phi, box_length, 0.195),
    math::Box2d({front_pose.x() + front_track_width_2 * sin_t, front_pose.y() - front_track_width_2 * cos_t}, front_pose.theta() + phi, box_length, 0.195),
  };
}
