...
bool DiscretePointsMath::ComputePathProfile(
  ...
  for (int i = 1; i < accumulated_s->size(); ++i) {
    speeds->push_back((accumulated_s->at(i) - accumulated_s->at(i - 1)) / dt);
  }
  double v = speeds->back();
  speeds->push_back(v);

  for (int i = 1; i < speeds->size(); ++i) {
    accelerations->push_back((speeds->at(i) - speeds->at(i - 1)) / dt);
  }
  double a = accelerations->back();
